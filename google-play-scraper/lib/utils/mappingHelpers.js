 ...
 *
 * FIXED: Added undefined checks to prevent TypeError
 * Issue: https://github.com/facundoolano/google-play-scraper/issues/720
 */
function extractCategories (searchArray, categories = []) {
  // FIXED: Added !searchArray check to handle undefined values
  ...
  if (searchArray.length >= 4 && typeof searchArray[0] === 'string') {
    ...
    searchArray.forEach((sub) => {
      // FIXED: Check if sub is defined before recursive call
      if (sub) {
